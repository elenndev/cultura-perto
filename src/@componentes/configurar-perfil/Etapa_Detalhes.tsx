...
export default function Etapa_Detalhes(props: etapaDetahesProps){
    ...
    function finalizarDetalhes(){
        if(Object.values(links).some(link => link.trim() !== "")){
            const linksDoPerfil: TypeLinksPerfil[] | null = Object.entries(links).filter(([_, link]) => link.trim() !== "").map(([nome, link]) => ({ nome: nome as TypeLinksPerfil["nome"], link }))


        } else {
            return setError('Por favor disponibilize ao menos um link')
        }
    }
